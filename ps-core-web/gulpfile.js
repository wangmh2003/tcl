...
var config = {
  ...
  uglifyexclude: ['!bower_components/**', '!node_modules/**', '!release/**', '!gulpfile.js', '!app-editor3/**', '!solution/**'],
  outDir: 'release',
  mapDir: 'map',
  devDir: 'css',
...
gulp.task('daily-version', ['copy-js'], function() {
...
gulp.task('release-version', ['minify-js'], function() {
  return gulp.src([config.outDir + '/**/js/main.js'])
    .pipe(plumber())
    .pipe(replace('==version==', (new Date()).getTime()))
    .pipe(gulp.dest(config.outDir));
